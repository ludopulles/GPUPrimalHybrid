...
def svp(
    ...
    # in g6k f = dim-kappa-eta (maybe need to edit)
    ...
    for d in range(1, search_space_dim + 1):
        ...
        for idxs_gpu in tqdm(
            ...
            for V_gpu in value_batches_fp32_gpu(vals_dev, d, VALUE_BATCH):
                ...
                idx = cp.where(cp.all((cp.abs(cp.rint(Y)) <= 15), axis=0))[
                    0
                ]  # find the error part
...
def drop_and_solve(lwe, params, iteration):
    ...
    if "k_dim" in params:
        ...
        columns_to_keep, mask = pick_columns_fast_ternary(lwe, params, _seed)
        good_count = int(np.count_nonzero(mask))
        if good_count < w - dim_needed:
            return SENT_FAIL0, SENT_FAIL1
        else:
            columns_to_keep.sort()
    # build the embedding
