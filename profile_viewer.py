...
try:
    from estimator import (
        Simulator,
    )  # expects: Simulator.CN11(d, n_eff, q, beta, xi=..., tau=..., dual=False)
except Exception:
    Simulator = None
...
def load_saved_profile(
    path: str, saved_form: str = "log2_norm2", scale: float = 1.0
) -> np.ndarray:
    """
...
def load_measured(
    path: str, measured_form: str = "log2", scaling_y: float = 1.0
) -> np.ndarray:
    raw = np.load(path)
...
def compute_cn11_curve(
    ...
    n_eff = n - zeta
    r = Simulator.CN11(
        d, n_eff, q, beta, xi=xi, tau=tau, dual=dual, max_loops=max_loops
    )  # squared norms
...
def plot_profiles(
    ...
    if cn11_enabled:
        if cn11_n is None or cn11_zeta is None or cn11_q is None:
            ...
            betas_auto = (
                sorted({p["beta"] for p in selection})
                if (selection and cn11_use_selection_betas)
                else []
            )
            betas_final = []
            if cn11_betas:
    ...
    for lbl, r in all_curves[1:]:
        diff = r - ref
        print(
            f"  {lbl:>18}: mean={diff.mean():.4f}, std={diff.std(ddof=1):.4f}, max|Δ|={np.abs(diff).max():.4f}"
        )
...
def interactive_shell(
    ...
    print(
        "  plot                              -> plot selection (+ measured/CN11 if enabled)"
    )
    print(
        "  scale set <hash> <s>              -> set per-hash scale (subtract log2(s))"
    )
    ...
    def cn11_show():
        ...
        print("\n[CN11 config]")
        print(
            f"  enabled={cfg['enabled']}, d={cfg['d']}, n={cfg['n']}, zeta={cfg['zeta']}, q={cfg['q']}"
        )
        print(
            f"  xi={cfg['xi']}, tau={cfg['tau']}, dual={cfg['dual']}, max_loops={cfg['max_loops']}"
        )
        print(
            f"  use_selection_betas={cfg['use_selection_betas']}, betas_explicit={sorted(cfg['betas'])}\n"
        )
    ...
    while True:
        ...
        if cmd == "list":
            ...
            if len(parts) >= 2 and parts[1] == "show":
                ...
                print(
                    "scale commands: 'scale show' | 'scale set <hash> <scale>' | 'scale clear <hash|all>'"
                )
            ...
            if sub == "on":
                ...
                if not db:
                    ...
                    for k in sorted(db.keys()):
                        cfg = db[k]
                        print(
                            f"- {k}: d={cfg.get('d')}, n={cfg.get('n')}, zeta={cfg.get('zeta')}, q={cfg.get('q')}, "
...
def parse_hash_scale_args(items: Optional[List[str]]) -> Dict[str, float]:
    ...
    for it in items:
        if "=" not in it:
            raise ValueError(
                "Bad --hash-scale format. Use HASH=SCALE and repeat as needed."
            )
        h, s = it.split("=", 1)
        h = h.strip().lower()
        s = float(s.strip())
...
def main():
    ...
    ap.add_argument("--list", action="store_true", help="List all profiles and exit")
    ap.add_argument(
        "--beta",
        type=int,
        nargs="*",
    ...
    ap.add_argument("--interactive", action="store_true", help="Interactive REPL")
    # Formats
    ap.add_argument(
        "--saved-form",
        default=None,
    ...
    ap.add_argument(
        "--xrange", type=str, default=None, help="Index range to plot, e.g., '10:200'"
    )
    # CN11 CLI
    ap.add_argument("--cn11", action="store_true", help="Enable CN11 overlay")
    ap.add_argument(
        "--cn11-d",
        type=int,
        default=None,
    ...
    ap.add_argument(
        "--cn11-n", type=int, default=None, help="CN11 parameter n (required if --cn11)"
    )
    ap.add_argument(
        "--cn11-zeta",
        type=int,
    ...
    ap.add_argument(
        "--cn11-q", type=int, default=None, help="CN11 modulus q (required if --cn11)"
    )
    ap.add_argument(
        "--cn11-betas",
        type=int,
    ...
    ap.add_argument(
        "--cn11-no-selection-betas",
        action="store_true",
        help="Do not use selection betas for CN11",
    )
    ap.add_argument(
        "--cn11-xi", type=float, default=1.0, help="CN11 xi parameter (default: 1.0)"
    )
    ap.add_argument(
        "--cn11-tau", type=float, default=0.0, help="CN11 tau parameter (default: 0.0)"
    )
    ap.add_argument("--cn11-dual", action="store_true", help="Enable CN11 dual mode")
    ap.add_argument(
        "--cn11-max_loops",
        type=int,
        default=8,
    ...
    if args.cn11_config_load:
        ...
        if (
            cfg.get("use_selection_betas") is not None
            and cfg.get("use_selection_betas") is False
        ):
